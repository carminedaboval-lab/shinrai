...
func _build_street_lights() -> void:
    ...
    for road_x: int in ROAD_CENTERS:
        for road_y: int in ROAD_CENTERS:
            if light_count >= MAX_STREET_LIGHTS:
                return
...
func _replace_visual_test_storefront_with_yakitori_shop() -> void:
    ...
    if old_root == null or old_threshold == null or old_lintel == null:
        push_warning("v10.27a Yakitori replacement skipped: locked storefront was not found")
        return

    var replacement: Node3D = YakitoriShopBuildingScene.instantiate() as Node3D
    if replacement == null:
        push_error("v10.27a Yakitori replacement failed: authored building scene could not instantiate")
        return

    var authored_model: Node3D = replacement.get_node_or_null("Model") as Node3D
    if authored_model == null:
        replacement.queue_free()
        push_error("v10.27a Yakitori replacement failed: authored Model root is missing")
        return
    ...
    print(
        "v10.27a authored Yakitori replacement: removed %s completely | installed %s | scale 1 | released interior lights %d" % [
            old_name,
            replacement.name,
