...
OUT_GLB = OUT_DIR / "ProjectShinrai_YakitoriShop_ArtworkReplica_v4.glb"
...
def build():
    ...
    keys=("plaster","seam","stone","exterior_wood","canopy_wood","upper_wood","frame_wood","glass","interior","roof","metal","lens","collision")
    ...
    for x in (-1.985,1.985):box("plaster",(x,1.47,-.615),(.23,2.58,.19),.012,tile=1.5);box("plaster",(x,4.34,-.615),(.23,2.62,.19),.012,tile=1.5)
    # Artwork joints are hairline formwork reveals, not black metal bars.
    for x in (-2.107,2.107):
        for z in (.48,1.70,2.92):box("seam",(x,2.92,z),(.006,4.78,.008),.001)
    for x in (-.72,.62):box("seam",(x,2.90,3.765),(.008,4.76,.006),.001)
    ...
    pitch=math.radians(-3.5);box("canopy_wood",(0,5.59,-.82),(4.36,.105,.38),.014,(pitch,0,0),swap=True);box("roof",(0,5.72,1.58),(4.40,.105,4.54),.012,(pitch,0,0),1.15);box("metal",(0,5.69,-1.015),(4.48,.075,.05),.006);box("metal",(0,5.755,-1.00),(4.52,.032,.11),.005)
    for x in (-2.215,2.215):box("metal",(x,5.73,1.57),(.04,.065,4.54),.005,(pitch,0,0))
    box("metal",(0,5.86,3.84),(4.46,.06,.05),.005)
    ...
    rx=1.62;rz=3.18;fz=.08;guard_h=.34
    def roof_top(z):return 5.72-math.sin(pitch)*(z-1.58)+.06
    for x in (-rx,rx):
    ...
    materials=[mat("MAT_Yakitori_04_PlasterConcrete","Plaster_BaseColor","Plaster_NormalGL","Plaster_ORM",0,.84,.22,.16),
      {"name":"MAT_Yakitori_04B_SubtleConcreteJoint","pbrMetallicRoughness":{"baseColorFactor":[.12,.115,.108,1],"metallicFactor":0,"roughnessFactor":.96},"extensions":{"KHR_materials_specular":{"specularFactor":.10}}},
    ...
    nodes += [{"name":name,"translation":pos} for name,pos in sockets];root=len(nodes);nodes.append({"name":"ProjectShinrai_YakitoriShop_ArtworkReplica_v4_ROOT","children":list(range(root)),"extras":{"PS_dimensions_m":[4.2,6.4,4.58],"PS_front_axis":"-Z","PS_building_only":True,"PS_excluded":["signage","lanterns","plants","props","AC units","finished interior"],"PS_placeholder_glass":True,"PS_game_ready":True,"PS_refinement":"subtle concrete joints, matte roof, darker stone, lighter eaves and roof guard"}})
    ...
    counts={k:len(g[k].arrays()[3])//3 for k in order};manifest={"asset":OUT_GLB.name,"version":"4.0","dimensions_m":[4.2,6.4,4.58],"front_axis":"-Z","scope":"building only","refinement":"close-up material and silhouette correction","excluded":["signage","lanterns","plants","props","AC units","finished interior"],"placeholder_glass":True,"triangle_counts":counts,"total_triangles_including_collision":sum(counts.values()),"material_slots":[m["name"] for m in materials],"sockets":[s[0] for s in sockets]};(OUT_DIR/"asset_manifest.json").write_text(json.dumps(manifest,indent=2)+"\n")
