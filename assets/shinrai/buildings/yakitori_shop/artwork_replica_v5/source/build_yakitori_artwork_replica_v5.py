#!/usr/bin/env python3
"""Build the refined SHINRAI Yakitori shop artwork replica as one GLB."""
...
OUT_GLB = OUT_DIR / "ProjectShinrai_YakitoriShop_ArtworkReplica_v5.glb"
...
def box_geom(center, size, bevel=0.0, rotation=(0,0,0), tile=0.72, swap_uv=False):
    ...
    uv_array=np.asarray(uv,np.float32)
    if swap_uv:uv_array=uv_array[:,::-1].copy()
    return p.astype(np.float32),normals.astype(np.float32),uv_array,np.asarray(idx,np.uint32)
...
def build():
    ...
    def retain(mesh, classify):
        ...
        for part in parts:
            ...
            swap=False
            if isinstance(target,tuple):target,swap=target
            if target:append_component(g[target],p,n,uv[:,::-1].copy() if swap else uv,t)
    def cedar(lo,hi):
        if lo[1]>=2.665:return None
        target="canopy_wood" if lo[1]>=2.40 else ("exterior_wood" if hi[1]<=.60 and (hi-lo)[0]>.20 else "frame_wood")
        return (target,True) if (hi-lo)[0]>3 and (hi-lo)[1]<.25 else target
    retain(2,cedar);retain(3,lambda lo,hi:"glass")
    retain(6,lambda lo,hi:None if np.all(lo>=(-.55,.55,-1.05)) and np.all(hi<=(.55,2.10,-.50)) else "metal")

    def box(k,c,s,b=0,r=(0,0,0),tile=.72,swap=False):g[k].add(*box_geom(c,s,b,r,tile,swap))
    def cyl(k,c,rad,h,seg=12,r=(0,0,0)):g[k].add(*cylinder_geom(c,rad,h,seg,r))
    front=-.60;rear=3.76;depth=rear-front;zc=(front+rear)/2;pitch=math.radians(-3.5)
    # Concrete shell. Large texture scale prevents obvious one-metre repetition.
    box("plaster",(-2.01,2.88,zc),(.18,5.44,depth),.012,tile=2.25);box("plaster",(2.01,2.88,zc),(.18,5.44,depth),.012,tile=2.25);box("plaster",(0,2.88,rear-.09),(4.2,5.44,.18),.012,tile=2.25)
    for x in (-1.985,1.985):box("plaster",(x,1.38,-.615),(.23,2.40,.19),.012,tile=1.5);box("plaster",(x,4.31,-.615),(.23,2.56,.19),.012,tile=1.5)
    # Continuous artwork wall: no applied joints, grooves, battens, or decals.
    # Sloped plaster closures make the wall meet the pitched roof with no open wedge.
    for x in (-2.01,2.01):box("plaster",(x,5.52,1.58),(.18,.22,4.18),.008,(pitch,0,0),2.25)
    box("plaster",(0,5.68,3.67),(4.20,.18,.18),.008,tile=2.25)
    for x in (-2.115,2.115):box("exterior_wood",(x,2.91,1.57),(.10,.14,4.35),.012,swap=True)
    box("exterior_wood",(0,2.91,3.865),(4.28,.14,.10),.012,swap=True)
    # Stone plinth.
    ...
    box("upper_wood",(0,3.13,-.70),(3.84,.15,.16),.016,swap=True);box("upper_wood",(0,5.49,-.70),(3.84,.15,.16),.016,swap=True)
    for x,w in ((-1.84,.17),(-.72,.12),(.58,.12),(1.84,.17)):box("frame_wood",(x,cy,-.715),(w,hh+.26,.16),.012)
    box("upper_wood",(1.20,bot+.04,-.805),(1.18,.075,.06),.008,swap=True);box("upper_wood",(1.20,top-.04,-.805),(1.18,.075,.06),.008,swap=True)
    ...
    # Re-authored storefront canopy: a shallow pitched skin, not the old thick balcony slab.
    canopy_pitch=math.radians(-2.0);box("roof",(0,2.68,-1.02),(4.32,.055,.72),.007,(canopy_pitch,0,0),1.05)
    box("metal",(0,2.655,-1.405),(4.40,.055,.045),.004);box("metal",(0,2.692,-.638),(4.30,.035,.035),.003)
    for x in (-2.18,2.18):box("metal",(x,2.68,-1.02),(.035,.045,.72),.003,(canopy_pitch,0,0))
    ...
    # Slim upper eave, matte low-slope roof, and separate dark flashing.
    box("canopy_wood",(0,5.57,-.82),(4.34,.095,.34),.012,(pitch,0,0),swap=True);box("roof",(0,5.68,1.57),(4.34,.085,4.42),.010,(pitch,0,0),1.15);box("metal",(0,5.54,-.965),(4.40,.065,.045),.005);box("metal",(0,5.59,-.95),(4.42,.028,.09),.004)
    for x in (-2.19,2.19):box("metal",(x,5.69,1.57),(.035,.055,4.42),.004,(pitch,0,0))
    box("metal",(0,5.84,3.79),(4.38,.05,.045),.004)
    # Three restrained soffit ribs, warm lenses, and lighter end brackets.
    for x in (-1.45,0,1.45):box("canopy_wood",(float(x),2.535,-1.03),(.045,.045,.62),.006)
    for x in (-1.32,0,1.32):cyl("lens",(x,2.515,-1.16),.085,.022,18)
    for x in (-1.93,1.93):box("canopy_wood",(x,2.40,-.91),(.072,.38,.075),.008,(math.radians(-34),0,0))
    # Rear window.
    ...
    # Asymmetric rear piers and a low, fine, roof-following guard.
    for x,h in ((-1.91,.74),(1.91,.58)):
        py=5.60+h/2;box("plaster",(x,py,3.48),(.25,h,.38),.012,tile=1);box("metal",(x,py+h/2+.025,3.48),(.29,.05,.42),.006)
    rx=1.60;rz=3.12;fz=.08;guard_h=.32
    def roof_top(z):return 5.68-math.sin(pitch)*(z-1.57)+.052
    for x in (-rx,rx):
        for z in np.linspace(fz,rz,3):
            z=float(z);box("metal",(x,roof_top(z)+guard_h/2,z),(.035,guard_h,.035),.003)
        for offset in (.13,.32):box("metal",(x,roof_top((fz+rz)/2)+offset,(fz+rz)/2),(.035,.035,rz-fz),.003,(pitch,0,0))
    for x in np.linspace(-rx,rx,4):box("metal",(float(x),roof_top(rz)+guard_h/2,rz),(.035,guard_h,.035),.003)
    for offset in (.13,.32):box("metal",(0,roof_top(rz)+offset,rz),(rx*2,.035,.035),.003)
    ...
    box("collision",(-2.01,2.88,zc),(.22,5.44,depth));box("collision",(2.01,2.88,zc),(.22,5.44,depth));box("collision",(0,2.88,rear-.09),(4.2,5.44,.22));box("collision",(-1.24,1.43,-.66),(1.30,2.46,.12));box("collision",(1.24,1.43,-.66),(1.30,2.46,.12));box("collision",(0,cy,-.66),(3.70,2.30,.12));box("collision",(0,5.68,1.57),(4.34,.14,4.42),r=(pitch,0,0))
    ...
    payloads=[("ExteriorWood_BaseColor",FILES["wood_base"].read_bytes()),
      ...
      ("Plaster_BaseColor",variant(FILES["plaster_base"],(0x78,0x76,0x6F),TEX_DIR/"Yakitori_PlasterConcrete_BaseColor.png")),("Plaster_NormalGL",FILES["plaster_normal"].read_bytes()),("Plaster_ORM",FILES["plaster_orm"].read_bytes()),
      ("Stone_BaseColor",variant(FILES["stone_base"],(0x6D,0x69,0x64),TEX_DIR/"Yakitori_StoneThreshold_BaseColor.png")),("Stone_NormalGL",FILES["stone_normal"].read_bytes()),("Stone_ORM",FILES["stone_orm"].read_bytes()),
      ("Roof_BaseColor",variant(FILES["roof_base"],(0x23,0x23,0x33),TEX_DIR/"Yakitori_RoofSurface_BaseColor.png")),("Roof_NormalGL",FILES["roof_normal"].read_bytes()),("Roof_ORM",FILES["roof_orm"].read_bytes())]
    order=list(keys);names={"plaster":"SM_YakitoriShop_ContinuousArtworkPlaster","stone":"SM_YakitoriShop_StoneThreshold","exterior_wood":"SM_YakitoriShop_ExteriorWoodMain","canopy_wood":"SM_YakitoriShop_CanopyEaveWood","upper_wood":"SM_YakitoriShop_UpperWoodCladding","frame_wood":"SM_YakitoriShop_WoodFrames","glass":"SM_YakitoriShop_GlazingPlaceholder","interior":"SM_YakitoriShop_InteriorPlaceholder","roof":"SM_YakitoriShop_MatteRoofSurface","metal":"SM_YakitoriShop_ArchitecturalMetal","lens":"SM_YakitoriShop_CanopyLightLenses","collision":"PS_YakitoriShop_ArtworkReplica_Collision-colonly"}
    ...
    materials=[mat("MAT_Yakitori_04_ContinuousPlasterConcrete","Plaster_BaseColor","Plaster_NormalGL","Plaster_ORM",0,.84,.22,.16),
      mat("MAT_Yakitori_05_StoneThreshold","Stone_BaseColor","Stone_NormalGL","Stone_ORM",0,.80,.28,.16),mat("MAT_Yakitori_01_ExteriorWoodMain","ExteriorWood_BaseColor","Wood_NormalGL","Wood_ORM",0,1,.48,.30),mat("MAT_Yakitori_02_CanopyEaveWood","CanopyEaveWood_BaseColor","Wood_NormalGL","Wood_ORM",0,.90,.40,.28),mat("MAT_Yakitori_03_UpperWoodCladding","UpperWoodCladding_BaseColor","Wood_NormalGL","Wood_ORM",0,1,.44,.28),mat("MAT_Yakitori_07_WoodFrames","WoodFrame_BaseColor","Wood_NormalGL","Wood_ORM",0,.92,.42,.32),
      ...
      mat("MAT_Yakitori_09_MatteRoofSurface","Roof_BaseColor","Roof_NormalGL","Roof_ORM",0,.88,.16,.18),
      {"name":"MAT_Yakitori_08_MetalTrimHandles","pbrMetallicRoughness":{"baseColorFactor":[.0103,.0103,.0103,1],"metallicFactor":.90,"roughnessFactor":.42},"extensions":{"KHR_materials_specular":{"specularFactor":.40}}},
      {"name":"MAT_Yakitori_CanopyLightLens","pbrMetallicRoughness":{"baseColorFactor":[1,.42,.16,1],"metallicFactor":0,"roughnessFactor":.28},"emissiveFactor":[1,.20,.045],"extensions":{"KHR_materials_emissive_strength":{"emissiveStrength":1.15}}}]
    ...
    nodes += [{"name":name,"translation":pos} for name,pos in sockets];root=len(nodes);nodes.append({"name":"ProjectShinrai_YakitoriShop_ArtworkReplica_v5_ROOT","children":list(range(root)),"extras":{"PS_dimensions_m":[4.2,6.4,4.58],"PS_front_axis":"-Z","PS_building_only":True,"PS_excluded":["signage","lanterns","plants","props","AC units","finished interior"],"PS_placeholder_glass":True,"PS_game_ready":True,"PS_refinement":"continuous plaster walls, closed pitched-roof junction, and rebuilt thin storefront canopy"}})
    ...
    counts={k:len(g[k].arrays()[3])//3 for k in order};manifest={"asset":OUT_GLB.name,"version":"5.0","dimensions_m":[4.2,6.4,4.58],"front_axis":"-Z","scope":"building only","refinement":"continuous artwork plaster, closed pitched-roof junction, and rebuilt thin storefront canopy","excluded":["signage","lanterns","plants","props","AC units","finished interior"],"placeholder_glass":True,"triangle_counts":counts,"total_triangles_including_collision":sum(counts.values()),"material_slots":[m["name"] for m in materials],"sockets":[s[0] for s in sockets]};(OUT_DIR/"asset_manifest.json").write_text(json.dumps(manifest,indent=2)+"\n")
